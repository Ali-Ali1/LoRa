...
from Framework.Node_2 import Node as Node_2
...
def run(locs, p_size, sigma, sim_time, gateway_location, num_nodes, transsmission_rate, confirmed_messages, adr):

    sim_env = simpy.Environment()
    gateway = Gateway(sim_env, gateway_location, max_snr_adr=True, avg_snr_adr=False)
    nodes = []
    energy_profile_n = EnergyProfile(5.7e-3, 15, tx_power_mW,
                                   rx_power=rx_measurements)
    lora_param_n = LoRaParameters(freq=np.random.choice(LoRaParameters.DEFAULT_CHANNELS),
                                sf=_sf,
                                bw=125, cr=5, crc_enabled=1, de_enabled=0, header_implicit_mode=0, tp=14)
    air_interface_2 = AirInterface(gateway, PropagationModel.LogShadow(std=sigma), SNRModel(), sim_env)
    node_2 = Node_2(1, energy_profile_n, lora_param_n, sleep_time=(8 * p_size / transmission_rate),
                    process_time=5,
                    adr=adr,
                    location=locs[1],
                    base_station=gateway, env=sim_env, payload_size=p_size, air_interface=air_interface_2,
                    confirmed_messages=confirmed_messages)
    air_interface_1 = AirInterface(node_2, PropagationModel.LogShadow(std=sigma), SNRModel(), sim_env)
    ...
    for node_id in range(num_nodes):
        ...
        node = Node(node_id, energy_profile, lora_param, sleep_time=(8 * p_size / transmission_rate),
                    ...
                    base_station=gateway, env=sim_env, payload_size=p_size, air_interface=air_interface_1,
                    confirmed_messages=confirmed_messages)
        nodes.append(node)
    ...
    return {
        ...
        'air_interface': data_air_interface, # Delete this one
        'path_loss_std': sigma,
        'payload_size': p_size,
